...
usage() {
    ...
    echo -e "\t-l\tSpecifies to use local socket connection."
...
# psql Command
########################################
if [ $PGLOCAL -eq 1 ]; then
    PSQL_CMD="/usr/bin/psql -U $PGUSER -d $DBNAME -A -t -c"
else
...
CLOUDWATCH_OPTS="--namespace aws-mon-pgsql --dimensions DBInstanceIdentifier=$(hostname)"
if [ -n "$PROFILE" ]; then
